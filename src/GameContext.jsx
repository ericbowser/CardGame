...
export const GameProvider = ({children}) => {
    ...
    const [lastBetAmount, setLastBetAmount] = useState(25); // Remember last bet for quick replay

    // Per-deck session stats (reset on shuffle — for basic strategy & card counting practice)
    ...
    const shuffleDeck = () => {
        ...
        // New deck — reset per-deck stats for basic strategy / counting practice
        setDeckWins(0);
        setDeckLosses(0);
        setDeckPushes(0);
        setDeckBlackjacks(0);
        addLog('Deck shuffled. Session stats reset.');
    };
    ...
    const handleWin = (blackjack = false) => {
        ...
        if (currentBet > 0) {
            if (blackjack) {
                ...
                setPlayerChips(prev => prev + currentBet + winnings);
                addLog(`Blackjack! Won $${winnings} (3:2 payout). Total: $${currentBet + winnings}`);
            } else {
                // Regular win pays 1:1
                const winnings = currentBet;
                setPlayerChips(prev => prev + currentBet + winnings);
                addLog(`Player wins! Won $${winnings}. Total: $${currentBet + winnings}`);
            }
            // Clear bet after payout
            setCurrentBet(0);
        }
    ...
    const handlePush = () => {
        ...
        if (currentBet > 0) {
            ...
            addLog(`Push - bet of $${currentBet} returned.`);
            // Clear bet after push
            setCurrentBet(0);
        }
        addLog('Push - round is a draw.');
    };

    // Place a bet before dealing
    const placeBet = (betAmount) => {
        ...
        setLastBetAmount(betAmount); // Remember this bet amount
        addLog(`Bet placed: $${betAmount}`);
        return true;
    };

    // Quick deal using last bet amount
    const quickDeal = () => {
        // Use last bet amount, but cap it at available chips
        const betToUse = Math.min(lastBetAmount, playerChips);
        if (betToUse <= 0) {
            setAlertMessage('Not enough chips to place a bet.');
            return;
        }
        if (placeBet(betToUse)) {
            handleDeal();
        }
    };
    ...
    const resetGame = () => {
        ...
        setPlayerChips(1000);
        setCurrentBet(0);
        setLastBetAmount(25); // Reset to default
        setDeckWins(0);
        setDeckLosses(0);
        setDeckPushes(0);
        setDeckBlackjacks(0);
    ...
    const value = {
        ...
        // Per-deck session stats (for basic strategy / card counting practice)
        deckWins,
        deckLosses,
        deckPushes,
        deckBlackjacks,
